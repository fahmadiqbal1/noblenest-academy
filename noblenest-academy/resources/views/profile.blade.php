        ...
        <div class="col-md-8">
            <div class="glass-panel p-4 {{ $isPlayful ? 'playful-font' : 'professional-font' }}">
                <div class="text-center">
                    ...
                    @if($role === 'Parent')
                        <h4 class="mt-4 mb-3" style="color:var(--nn-primary);"><i class="bi bi-people"></i> {{ I18n::get('your_children') }}</h4>
                        <ul class="list-group mb-3">
                            @forelse($user->children ?? [] as $child)
                                <li class="list-group-item d-flex align-items-center justify-content-between"
                                    ...
                                    <span class="badge" style="background:var(--nn-success);">{{ $child->progress ?? '0%' }}</span>
                                    <a href="/children/{{ $child->id }}/edit" class="btn btn-sm btn-outline-secondary ms-2"><i class="bi bi-pencil"></i></a>
                                </li>
                            @empty
                                <li class="list-group-item" style="color:var(--nn-text-muted);">{{ I18n::get('no_children') ?? 'No children added yet.' }}</li>
                            @endforelse
                        </ul>
                        ...
                        <div class="mt-4">
                            <h5 style="color:var(--nn-primary);"><i class="bi bi-bar-chart-line"></i> {{ I18n::get('analytics') }}</h5>
                            <div class="progress mb-2" style="height:1.2rem;border-radius:var(--nn-radius-sm);">
                        ...
                        <div class="mt-4">
                            <h5 style="color:var(--nn-text-muted);"><i class="bi bi-person-badge"></i> {{ I18n::get('customize_avatar') ?? 'Customize Avatar' }}</h5>
                        ...
                        <div class="mt-4">
                            <h5 style="color:var(--nn-success);"><i class="bi bi-lightbulb"></i> {{ I18n::get('recommendations') ?? 'Recommendations' }}</h5>
                            <ul class="list-group mb-2">
                                <li class="list-group-item" style="border-color:var(--nn-border);">{{ I18n::get('weekly_summary') ?? 'Your child completed 3 new activities this week!' }}</li>
                                <li class="list-group-item" style="border-color:var(--nn-border);">{{ I18n::get('suggested_module') ?? 'Try the new tracing activity for fine motor skills.' }}</li>
                                <li class="list-group-item" style="border-color:var(--nn-border);">{{ I18n::get('parent_tip') ?? 'Check out the parent resources for emotional coaching.' }}</li>
                            </ul>
                        </div>
                    @elseif($role === 'Student')
                        <div class="mb-4">
                            <span class="badge fs-5" style="background:var(--nn-primary);">{{ $user->progress ?? '0%' }} Complete</span>
                            <div class="progress mt-2" style="height:1.5rem;border-radius:var(--nn-radius-sm);">
                                <div class="progress-bar" role="progressbar"
                                     style="width:{{ $user->progress ?? 0 }}%;background:var(--nn-primary);">{{ $user->progress ?? 0 }}%</div>
                            </div>
                        </div>
                        <div class="mb-3">
                            ...
                            <span class="badge" style="background:var(--nn-primary);"><i class="bi bi-trophy"></i> {{ $user->achievements ?? 0 }} Achievements</span>
                        </div>
                        <div class="mb-4">
                            <h5 style="color:var(--nn-primary);"><i class="bi bi-map"></i> {{ I18n::get('progress_map') }}</h5>
                        ...
                        <a href="/activities" class="btn btn-primary btn-lg mt-2"><i class="bi bi-controller"></i> {{ I18n::get('continue_learning') }}</a>
                    @endif
                    <div class="mt-4">
