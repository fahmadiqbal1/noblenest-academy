...
<div class="flex flex-col lg:flex-row -m-8 overflow-hidden rounded-[var(--radius-card)]">
  ...
  <aside class="lg:w-2/5 p-8 text-white bg-gradient-to-b from-[var(--color-brand-700)] via-[var(--color-brand-600)] to-[var(--color-brand-400)]">
    ...
    <p class="text-xs font-extrabold uppercase tracking-widest text-white/80 mb-2">{{ __('auth.register_get_started') }}</p>
    <h1 class="text-2xl font-bold font-[var(--font-display)] mb-3">{{ __('auth.register') }}</h1>
    ...
    <div class="space-y-3">
      <div class="rounded-[var(--radius-sm)] p-3 bg-white/12 border border-white/18">
        <div class="flex items-center gap-2 mb-1">
          ...
          <span class="font-semibold text-sm">{{ __('auth.register_parents') }}</span>
        </div>
        <p class="text-xs text-white/78">{{ __('auth.register_parents_desc') }}</p>
      </div>
    </div>
  </aside>
  ...
  <div class="flex-1 p-8"
    ...
    <h2 class="text-2xl font-bold text-[var(--color-text)] font-[var(--font-display)] mb-1">{{ __('auth.register_heading') }}</h2>
    <p class="text-sm text-[var(--color-text-muted)] mb-6">{{ __('auth.register_subheading') }}</p>
    ...
    <form method="POST" action="{{ url('/register') }}" @submit="loading = true" novalidate>
      ...
      {{-- Role picker --}}
      <fieldset class="mb-5">
        <legend class="block text-sm font-semibold text-[var(--color-text)] mb-2">{{ __('auth.register_as') }}</legend>
        <div class="grid grid-cols-2 gap-2">

          @php
          $roles = [
            'Parent' => ['icon' => 'users', 'desc' => __('auth.role_parent_desc')],
          ];
          @endphp
      ...
      <div class="space-y-4">
        ...
        <x-ui.field name="name" label="{{ __('auth.name') }}" :error="$errors->first('name')" required>
          <x-ui.input
            type="text"
            name="name"
            :value="old('name')"
        ...
        <x-ui.field name="password" label="{{ __('auth.password') }}" :error="$errors->first('password')" required>
        ...
        <x-ui.field name="password_confirmation" label="{{ __('auth.confirm_password') }}" :error="$errors->first('password_confirmation')" required>
          <div class="relative flex">
            <input
        ...
        <div class="flex items-start gap-3">
          ...
          <label for="terms" class="text-sm text-[var(--color-text-muted)] cursor-pointer select-none leading-snug">
            {{ __('auth.terms_agree') }}
            <a href="{{ url('/terms') }}" target="_blank" class="font-semibold text-[var(--color-primary)] hover:underline focus-visible:outline-2 focus-visible:outline-[var(--color-brand-600)] focus-visible:outline-offset-2 rounded">{{ __('auth.terms_of_service') }}</a>
            {{ __('auth.terms_and') }}
            <a href="{{ url('/privacy') }}" target="_blank" class="font-semibold text-[var(--color-primary)] hover:underline focus-visible:outline-2 focus-visible:outline-[var(--color-brand-600)] focus-visible:outline-offset-2 rounded">{{ __('auth.privacy_policy') }}</a>.
          </label>
        </div>
        ...
        >
          <span x-show="!loading">{{ __('auth.register') }}</span>
          <span x-show="loading" x-cloak>{{ __('auth.creating_account') }}</span>
        </x-ui.button>
