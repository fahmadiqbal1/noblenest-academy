    ...
    <div class="container mx-auto px-4">
        <nav class="flex items-center h-16 gap-4" aria-label="Parent navigation">
            ...
            <ul class="hidden lg:flex items-center gap-1 ml-4 flex-1" role="list">
                ...
                <li><a href="/children" class="px-3 py-2 rounded-[var(--radius-sm)] text-sm font-bold text-[var(--color-text-muted)] hover:bg-[var(--color-primary-soft)] hover:text-[var(--color-primary)] transition-colors {{ request()->is('children*') ? 'bg-[var(--color-primary-soft)] text-[var(--color-primary)]' : '' }}">{{ __('common.nav_children') }}</a></li>
...
<nav class="lg:hidden fixed bottom-0 inset-x-0 z-30 bg-white/92 backdrop-blur-xl border-t-[3px] border-[var(--color-border)] shadow-[0_-4px_16px_rgba(30,41,59,0.06)]" aria-label="Mobile navigation">
    <div class="flex items-center justify-around py-2">
        ...
        <a href="/children" class="flex flex-col items-center gap-0.5 px-4 py-1 text-xs font-bold {{ request()->is('children*') ? 'text-[var(--color-primary)]' : 'text-[var(--color-text-muted)]' }} transition-colors hover:text-[var(--color-primary)]">
        ...
        <a href="/profile" class="flex flex-col items-center gap-0.5 px-4 py-1 text-xs font-bold {{ request()->is('profile') ? 'text-[var(--color-primary)]' : 'text-[var(--color-text-muted)]' }} transition-colors hover:text-[var(--color-primary)]">
