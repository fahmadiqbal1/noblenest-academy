...
class CurriculumController extends Controller
{
    ...
    // SCHEDULED DAILY REPORTS: Send daily activity summary to admin via email
    public function sendDailyReport()
    {
        $today = now()->format('Y-m-d');
        $activities = Activity::whereDate('created_at', $today)->get();
        if ($activities->isEmpty()) {
            return response()->json(['message' => 'No new activities today.'], 204);
        }
        // Send email to admin(s)
        \Mail::send('admin.activities.daily_report_email', compact('activities'), function($m) {
            $m->to(config('mail.admin_address', '
